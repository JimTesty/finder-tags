...
and `--reverse`. Without `--space-indent`, one- and two-tag outputs receive
extra spaces so their initials align with three-tag output; `--align-tags`
includes those spaces in its per-file minimum width.
